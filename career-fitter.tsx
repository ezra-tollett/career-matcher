...
const CareerFitter = () => {
  ...
  const handleSelectMBTI = (type) => {
    setProfile(prev => ({ ...prev, mbti: type }));
    setStep(1);
  };

  const handleSelectHolland = (code) => {
    setProfile(prev => ({ ...prev, hollandCode: code }));
    setStep(2);
  };

  const handleSelectIntelligence = (intelligence) => {
    setProfile(prev => {
  ...
  return (
    <Card className="w-full max-w-3xl mx-auto">
      ...
      <CardContent>
        {!results && (
          <Progress
            value={((step + (profile.intelligences.length / 3)) / 3) * 100}
            className="mb-6"
          />
        )}
        {results ? renderResults() : renderStep()}
      </CardContent>
    </Card>
  );
};
